...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        super().__init__(screen, title or "Prepare")
        ...
        for tool in range(4):
            for line, (key, text, style, diameter) in enumerate((
                ("dock", "Dock", "color1", None),
                ("park", "Park", "color2", None),
                ("prime", "Prime", "color3", None),
            )):
        ...
        temperature_button = self._gtk.Button("heat-up", "Temperature", "color2", scale=0.8)
        ...
        for col, diameter in ((0, 1.75), (2, 2.85)):
            button = self._gtk.Button(label=f"{diameter}mm Extrude", style="color4")
            button.connect("clicked", self._extrude_current, diameter)
            actions.attach(button, col, 4, 2, 1)
        motor = self._gtk.Button("motor-off", "Motors Off", "color2", scale=0.8,
        ...
        self._unknown("Waiting for status")
    ...
    def _compact_message(message, columns=28):
        ...
        if len(lines) > 4:
            lines = lines[:3] + ["See log for details"]
        return "\n".join(lines)
    ...
    def _indicate_tool(self, tool):
        ...
        self.labels["selected"].set_text(f"Active tool: T{tool + 1}" if tool is not None else "Active tool: —")
    ...
    def _action_clicked(self, widget, action, tool, diameter):
        if self._action_pending and action != "motor_off":
            self._notify("Action in progress.\nPlease wait.")
            return
        if not self._active or time.monotonic() - self._status_time > 5:
            self._notify("Status unavailable.\nCheck the connection.")
            return
        ...
        if not self._printer.get_config_section("gcode_macro PREPARE_ACTION"):
            self._notify("Install preparation macros,\nthen restart Klipper.")
            return
        ...
        try:
            ...
            if sent is False:
                self._action_pending = False
                self._unknown("Check the connection")
                self._notify("Failed to send command.")
            ...
            self._unknown("Check command result")
            self._notify("Result unavailable.\nCheck the printer.")
    ...
    def _action_done(self, response, method, params):
        ...
        self._generation += 1
        self._pending = False
        self._unknown("Waiting for status update")
    ...
    def _render_mount(self):
        ...
        if state.get("changing"):
            ...
            text = "Unknown"
        self.labels["mount"].set_text(f"Mount status: {text}")

    def activate(self):
        self.deactivate()
        self._active = True
        self._unknown("Checking status")
        self._poll()
    ...
    def _unknown(self, message):
        ...
        self._display_pending_tool = None
        self._indicate_tool(None)
        self.labels["mount"].set_text(f"Mount status: {message}")
        self.labels["temperature"].set_text("Nozzle temperature: — / — ℃")
    ...
    def _poll(self):
        ...
        if not socket or not getattr(socket, "connected", False):
            self._generation += 1
            self._pending = False
            self._unknown("Waiting for printer connection")
            return True
        if self._pending:
            if time.monotonic() - self._requested_at > 8:
                self._unknown("Response delayed; status unavailable")
            return True
        ...
        try:
            ...
            if sent is False:
                self._pending = False
                self._unknown("Status query failed")
        except Exception:
    ...
    def _status_received(self, response, method, params, generation):
        ...
        if "error" in response or status.get("webhooks", {}).get("state") != "ready":
            self._unknown("Check printer readiness")
            return False
        self._status = status
        ...
        self.labels["temperature"].set_text(f"Nozzle temperature: {current} / {target} ℃")
        ...
        if error and not self._reported_error:
            self._notify("No nozzle detected.\nCheck the toolhead.")
